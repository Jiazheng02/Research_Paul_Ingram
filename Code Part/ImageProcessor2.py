...
class ImageProcessor:
    ...
    @staticmethod
    def are_lines_similar(line1, line2, length_threshold=15, midpoint_threshold=15, angle_threshold=3):
        """
    ...
    def _merge_similar_nearby_lines(self, lines, point_threshold=15, angle_threshold=3):
        """
            ...
            for i in range(len(merged_lines)):
                ...
                for j in range(i + 1, len(merged_lines)):
                    if not used[j]:
                        ...
                        if self.angle_between_lines(line1, line2) < angle_threshold:
                            p1, p2 = np.array(line1[:2]), np.array(line1[2:])
                            q1, q2 = np.array(line2[:2]), np.array(line2[2:])

                            # Check if the lines are close enough
                            if np.linalg.norm(p1 - q1) < point_threshold:
                                merged_segment = list(line1[:2]) + list(line2[2:])  # Connect p1 to q2
                                used[j] = True
                                merged = True
                            elif np.linalg.norm(p1 - q2) < point_threshold:
                                merged_segment = list(line1[:2]) + list(line2[:2])  # Connect p1 to q1
                                used[j] = True
                                merged = True
                            elif np.linalg.norm(p2 - q1) < point_threshold:
                                merged_segment = list(line1[2:]) + list(line2[2:])  # Connect p2 to q2
                                used[j] = True
                                merged = True
                            elif np.linalg.norm(p2 - q2) < point_threshold:
                                merged_segment = list(line1[2:]) + list(line2[:2])  # Connect p2 to q1
                                used[j] = True
                                merged = True
    ...
    def find_lines(self):
        ...
        lines = [self.normalize_line(line) for line in lines]  # Normalize line coordinates
    ...
    def match_relationships(self):
        ...
        for line in self.lines:
            ...
            # Determine the start (bottom) and end (top) points of the line
            if y0 > y1:
                start_point = (x0, y0)
                end_point = (x1, y1)
            else:
            ...
            # Find the nearest text box for the start (from) and end (to) points
            text_from, center_from = self._find_nearest_textbox(start_point[0], start_point[1])
            text_to, center_to = self._find_nearest_textbox(end_point[0], end_point[1])

            # Ensure valid matches and that the two endpoints are associated with different text boxes
